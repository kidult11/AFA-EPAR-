...
def train(cfg):
    # 修正2：调低num_workers，适配单卡机器（8核=4，12核=6）
    ...
    setup_seed(42)  # 固定种子，可选（建议开启，保证复现）
    ...
    train_loader = DataLoader(train_dataset,
                              ...
                              #//sampler=train_sampler,# 单卡注释分布式采样器
                              prefetch_factor=2)  # 修正2：16G内存设2，32G+设4
...
if __name__ == "__main__":
    ...
    os.makedirs(cfg.work_dir.segs_dir, exist_ok=True)
    os.makedirs(cfg.work_dir.tb_logger_dir, exist_ok=True)

    # 修正1：删除local_rank判断，单卡直接初始化日志
