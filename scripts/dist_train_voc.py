...
parser.add_argument("--local_rank", default=-1, type=int, help="local_rank")   # 分布式进程编号（单卡需删除或默认-1，保留兼容）
...
def train(cfg):
    ...
    #num_workers = 10 # 数据加载线程数，8核设为4，16核设为6
    num_workers = 10 # 数据加载线程数，8核设为4，16核设为6.,a100 20
    # ---------------------- 新增：梯度累积参数 ----------------------
    accumulation_steps = 2  # 每2步更新一次，等效总batch=2×2=4
    # -----------------------------------------------------------------
    ...
    train_loader = DataLoader(train_dataset,
                              ...
                              #//sampler=train_sampler,# 丢弃最后一个不完整批次,单卡的话注释来一处分布式采样器
                              #//prefetch_factor=4       # 丢弃最后一个不完整批次
                              prefetch_factor=4)  #避免内存占用过高，内存一般推荐2
    ...
    logging.info('\nNetwork config: \n%s'%(wetr))# 打印模型结构
    # 在 logging.info(...) 后添加,确认预训练权重真的加载成功（而非随机初始化）;参数均值≈0 → 权重未加载,参数均值≠0 → 权重加载成功。
    for name, param in wetr.encoder.named_parameters():
        if "norm" not in name and param.requires_grad:
            print(f"Backbone layer: {name}, Param mean: {param.data.mean().item():.6f}")
            break  # 只打印第一个骨干层参数，避免刷屏
    ...
    # 移除local_rank判断，直接创建SummaryWriter    
    writer = SummaryWriter(cfg.work_dir.tb_logger_dir)
    dummy_input = torch.rand(1, 3, 384, 384).to(device)
    # writer.add_graph(wetr, dummy_input)  # 可选，可视化模型
    ...
    for n_iter in range(cfg.train.max_iters):
        ...
        # ---------------------- 修改：损失缩放 ----------------------
        loss = loss / accumulation_steps  # 梯度缩放，避免累积后梯度过大
        # -----------------------------------------------------------
        ...
        if (n_iter+1) % cfg.train.log_iters == 0:
            ...
            seg_mAcc = (preds==gts).sum() / preds_size if preds_size != 0 else 0.0
            ...
            # 移除local_rank判断，直接打印日志和写tensorboard
            logging.info("Iter: %d; Elasped: %s; ETA: %s; LR: %.3e; cls_loss: %.4f, aff_loss: %.4f, pseudo_seg_loss %.4f, pseudo_seg_mAcc: %.4f"%(n_iter+1, delta, eta, cur_lr, avg_meter.pop('cls_loss'), avg_meter.pop('aff_loss'), avg_meter.pop('seg_loss'), seg_mAcc))
...
if __name__ == "__main__":
    ...
    if args.work_dir is not None:
        ...
        cfg.work_dir.ckpt_dir = os.path.join(args.work_dir, "checkpoints")
        cfg.work_dir.pred_dir = os.path.join(args.work_dir, "predictions")
        cfg.work_dir.segs_dir = os.path.join(args.work_dir, "segs")
        cfg.work_dir.tb_logger_dir = os.path.join(args.work_dir, "tb_logger")
    ...
    # 给模型保存/日志目录加上时间戳，避免不同次训练的文件覆盖

    
    # 单卡正确代码（精简，无重复）
    os.makedirs(cfg.work_dir.dir, exist_ok=True) # 根目录
    os.makedirs(cfg.work_dir.ckpt_dir, exist_ok=True) # 模型保存目录
    os.makedirs(cfg.work_dir.pred_dir, exist_ok=True) # 预测结果目录
    os.makedirs(cfg.work_dir.segs_dir, exist_ok=True) # 分割结果目录
    os.makedirs(cfg.work_dir.tb_logger_dir, exist_ok=True) # TensorBoard日志目录
